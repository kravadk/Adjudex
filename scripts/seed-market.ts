// Creates a market on-chain from MARKET_SPEC_JSON.
//
...
//
// Run:
// MARKET_SPEC_JSON='{"title":"...","description":"...","category":"crypto","oracleType":"manual","asset":"USDC","deadlineIso":"2026-07-01T00:00:00.000Z","feeBps":100,"sourceUrl":"https://...","resolutionCriteria":"..."}' pnpm tsx --env-file=.env.local scripts/seed-market.ts
...
async function main() {
  ...
  if (!privateKey || !rpcUrl || !factoryAddress) {
    throw new Error(
      "Set DEPLOYER_PRIVATE_KEY, ARBITRUM_SEPOLIA_RPC_URL and MARKET_FACTORY_ADDRESS in .env.local.",
    );
  }
  ...
  const hash = await walletClient.writeContract({
    ...
    args: [specHash, deadlineSec, specUri],
  });
  console.log(`tx: ${hash}`);

  const receipt = await publicClient.waitForTransactionReceipt({ hash });
  if (receipt.status !== "success") throw new Error(`createMarketWithSpec reverted: ${hash}`);
