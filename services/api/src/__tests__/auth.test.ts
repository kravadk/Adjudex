...
describe("auth helpers", () => {
  it("builds a wallet ownership message with nonce and domain", () => {
    ...
    expect(message).toContain("pariai.test wants you to sign in with your Ethereum account");
  ...
  it("normalizes the configured SIWE domain from a canonical app URL", () => {
    expect(configuredSiweDomain({ NEXT_PUBLIC_APP_URL: "https://Auth.PariAI.test/app" })).toBe("auth.pariai.test");
  });

  it("falls back to the production SIWE domain when config is missing", () => {
    expect(configuredSiweDomain({})).toBe("pariai.app");
  });
  ...
  it("marks session cookies secure in production and expires the same cookie on logout", () => {
    ...
    expect(expiredSessionCookie()).toBe("pariai_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0; Secure");
